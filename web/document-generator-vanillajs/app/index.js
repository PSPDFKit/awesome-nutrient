...
function startTransition(message) {
  const activeSections = Array.from(sections).filter(section => !section.classList.contains("none"));
  ...
  transitionSection.getElementsByTagName('h2')[0].innerText = message;
...
function goTemplateEditor() {
  ...
  const editorElement = editTemplateSection.getElementsByClassName('nutri-editor')[0];  
  (async () => {
    // init doc. authoring
    ...
    if(APP.templateDocument === null) {
      const templateDocument = (APP.template == 'custom') ? 
        ...
        : await APP.docAuthSystem.loadDocument(
            await fetch(`${backendUrl}templates/${APP.template}.json`).then(response => response.json())
          ); // or load preselected template DocJSON      
      APP.templateDocument = templateDocument;
    }

    // initialize editor
    const editor = await APP.docAuthSystem.createEditor(editorElement, { document: APP.templateDocument });
...
function initoDataEditor() {
  const doButtonAction = e => {              
    let textarea = APP.dataEditor.getTextArea();
...
function goDataEditor() {
  ...
  (async () => {
    ...
    if(APP.dataJson === null) {
      const dataJson = await fetch(`${backendUrl}data/${APP.template}.json`).then(response => response.json());
...
const editGeneratedDocxSection = document.getElementById('Step4_EditGeneratedDocx');
...
(() => {
  ...
  document.getElementById('theTitle').addEventListener("click", e => { window.location.reload(); });
