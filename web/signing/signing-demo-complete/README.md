...
3. Run `npm i` to install the project dependencies. This example also ships a pnpm setup (`packageManager: pnpm@11.20.0` plus `pnpm-lock.yaml`); if you have enabled corepack's npm shim (`corepack enable npm`), corepack will refuse `npm` commands here — use `pnpm install` instead (and `pnpm` in the steps below), or run npm with `COREPACK_ENABLE_STRICT=0`.
4. Copy the Nutrient Web SDK library assets to the public directory by running:
`cp -R ./node_modules/pspdfkit/dist/pspdfkit-lib public/pspdfkit-lib`
(the `./node_modules` path works with either package manager)
5. You should now be able to run the project locally by executing `npm run dev` (or `pnpm dev`).
